...
- Новые фотографии для публичных страниц добавлять в `img/` уже в формате WebP. Если исходник пришел из Figma/JPEG/PNG, сначала конвертировать в `.webp`, затем подключать на странице.
- Перед добавлением фото из Figma проверить дубли: часть кадров ДВЦ уже лежит в `img/venue-*`, повторные фасады и залы не коммитить.
...
## Страница экспонентов (exhibitors.html) в админке

- В админке есть вкладка «Страница экспонентов» (переключатель вверху `admin.html`), отдельная от вкладки «Главная страница».
- Весь текст `exhibitors.html` (герой, «О выставке», блоки ценности/аудитории, тарифы, «Застройка стенда», бизнес-карточки, площадка, FAQ, блок заявки, подвал, SEO) хранится в `content.texts.exhibitorsPage` и редактируется через эту вкладку.
- Список экспонентов (карточки участников) **общий** для `index.html` и `exhibitors.html` — редактируется только на вкладке «Главная страница» (`content.participants`), отдельного списка на странице экспонентов нет.
- В `exhibitors.html` текстовые узлы помечены атрибутами `data-content="expXxx"` (одиночные поля) и `data-exp-*="N"` (элементы списков фиксированной длины: показатели героя, карточки тарифов, вопросы FAQ и т.д.). `content-loader.js` подставляет значения по этим атрибутам через функции `applyExp*`; если поля в сохранённом `content.json` ещё нет — просто остаётся статический текст из HTML, ничего не ломается.
- Списки-буллеты (пункты тарифа, состав стандартной застройки) хранятся как одна строка с `\n` между пунктами, а не как массив — так проще биндить в `<textarea>` в админке.
- Все списки на странице экспонентов **фиксированной длины** (без кнопок добавить/удалить) — количество тарифов/вопросов FAQ/карточек жёстко зашито и в HTML, и в схеме `content.seed.json`. Если нужно добавить новый пункт (например, 14-й вопрос FAQ), потребуется руками добавить элемент и в `exhibitors.html`, и в `content.seed.json`/`defaultExhibitorsPage` в `admin.html`.
- `content.seed.json` — это только стартовые данные для первого запуска сервера (когда `/app/data/content.json` ещё не существует). На проде, где `content.json` уже есть, правки `content.seed.json` **не применяются** — если нужно поменять текст на живом сайте, это делается через саму админку.
